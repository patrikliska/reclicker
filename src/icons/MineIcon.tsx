import * as React from 'react';

function MineIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      version='1.1'
      id='Capa_1'
      xmlns='http://www.w3.org/2000/svg'
      xmlnsXlink='http://www.w3.org/1999/xlink'
      x='0px'
      y='0px'
      viewBox='0 0 461.997 461.997'
      enableBackground='new 0 0 461.997 461.997;'
      xmlSpace='preserve'
      {...props}
    >
      <g>
        <polygon
          fill='#CCCCCC'
          points='180.658,329.168 186.788,358.168 128.788,358.168 134.908,329.168 158.328,329.168 	'
        />
        <polygon
          fill='#E6E6E6'
          points='335.918,340.168 339.728,358.168 303.728,358.168 307.528,340.168 321.348,340.168 	'
        />
        <polygon
          fill='#666666'
          points='313.888,301.148 321.348,340.168 307.528,340.168 303.728,358.168 186.788,358.168 
     180.658,329.168 158.328,329.168 163.688,301.148 192.308,288.138 208.968,249.968 266.438,249.968 286.748,286.638 	'
        />
        <polygon
          fill='#CCCCCC'
          points='185.788,128.168 200.878,103.778 282.368,90.828 292.788,128.168 307.898,128.168 
     325.648,198.168 359.118,198.168 382.788,276.168 415.858,276.168 440.998,358.168 339.728,358.168 335.918,340.168 
     321.348,340.168 313.888,301.148 286.748,286.638 266.438,249.968 208.968,249.968 192.308,288.138 163.688,301.148 
     158.328,329.168 134.908,329.168 128.788,358.168 20.998,358.168 46.138,276.168 91.978,276.168 115.648,198.168 136.348,198.168 
     154.098,128.168 	'
        />
        <polygon
          fill='#B3B3B3'
          points='136.349,198.172 179.788,198.172 139.61,185.305 	'
        />
        <polygon
          fill='#B3B3B3'
          points='91.978,276.168 83.237,305.002 78.788,276.168 	'
        />
        <polygon
          fill='#B3B3B3'
          points='325.644,198.172 333.087,227.531 336.122,198.168 	'
        />
        <polygon
          fill='#B3B3B3'
          points='382.785,276.172 315.788,276.172 379.154,264.193 	'
        />
        <polygon
          fill='#B3B3B3'
          points='223.885,198.172 263.885,198.172 233.455,190.102 	'
        />
        <polygon
          fill='#B3B3B3'
          points='293.219,128.172 214.552,128.172 274.398,120.102 	'
        />
        <polygon
          fill='#E6E6E6'
          points='149.455,329.168 153.455,358.168 128.788,358.168 134.908,329.168 	'
        />
        <path
          fill='#808080'
          d='M461.997,371.168H0v0c0-7.18,6.097-13,13.619-13h434.759
     C455.899,358.168,461.997,363.989,461.997,371.168L461.997,371.168z'
        />
      </g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
      <g></g>
    </svg>
  );
}

export default MineIcon;
